...
- [`yay`](https://github.com/Jguer/yay) ou [`paru`](https://github.com/morganamilo/paru) (optionnel, pour l'AUR — yay prioritaire)
...
- `TIMEOUT=30` : timeout (secondes) pour `checkupdates` et `yay`/`paru -Qum`
- `NOCONFIRM=1` : ajoute `--noconfirm` a `yay`/`paru`/`pacman`, `--no-confirm` a `pamac`
- `UPDATE_ENABLE_PIP=1` : inclut la mise a jour de pip (deconseille sur Python systeme)
...
update update   # lance eos-rankmirrors, yay/paru, pamac et pip
```
...
- Si `yay` et `paru` sont absents, le script ignore simplement l'AUR.
